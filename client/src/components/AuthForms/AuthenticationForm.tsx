...
const AuthenticationForm = () => {
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			const res = await (isSignIn ? signin(user as { email: string; password: string }) : signup(user));
			...
			toast.error(<ToastContent res={isSignIn ? 'Sign In failed' : 'Sign Up failed'} messages={error.response?.data?.errors || ['An error occurred']} />);
		}
	};
							...
							<div className={classes['input-container']}>
								<input
									...
									onChange={(e) => {
										...
										setPgPercentage((Number(pgCgpaValue) * 9.5).toFixed(2));
									}}
									step="0.01"
								/>
								<div>CGPA</div>
